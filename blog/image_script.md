+++
date = "2016-02-03T20:27:15Z"
title = "A script to generate android images"
...
You can find the script here [process.vbs](https://github.com/ernan/vbscripts/process.vbs "Process SVG Images for Android")

So I build all my images in [inkscape](https://inkscape.org/en/download/ "inkscape"). 

This is brilliant application.

You can find some brilliant tutorials here [heathenx](http://screencasters.heathenx.org/ "heathenx")

The script uses [inkscape](https://inkscape.org/en/download/ "inkscape") to convert the svg files to png images of 
different sizes.

The script also use [imagemajic](http://www.imagemagick.org/script/binary-releases.php "ImageMagic") to format the pngs nicely.

Finally it compresses the result using two png crushing programs

[pngquant](https://pngquant.org/ "pngquant") and [pngout](http://advsys.net/ken/utils.htm "pngout")

You will need to put these two binaries into your path.

So the general set up will be

1. Put an "images" folder in your project directory.
2. Run the script on using _**cscript process.vbs**_
3. The script will look for svg images in the images directory that have changed in the past 10 minutes.
4. It will generate images of different sizes in different parts of your android application
5. There  is a mapping table which governs what size image will be generated and where depending upog the name of the svg file.
    for example if the svg file starts with ic_menu* the script will generate menu icon sizes from the svg file.
